	...
	if ($stmt->rowCount () >= 1)
	{
		...
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)) 
		{
				...
				echo	'<tr>
						...
						<tr>
							<td colspan="2"></td>
							<td>A. <input type="radio" id="A" onClick="Mark_choice_current_question()" name="'.$row['id'].'" value="A" >'."	".$row['Option_1'].'</input></br>
							B. <input type="radio" id="B" onClick="Mark_choice_current_question()" name="'.$row['id'].'" value="B" >'."	".$row['Option_2'].'</input></br>
							C. <input type="radio" id="C" onClick="Mark_choice_current_question()" name="'.$row['id'].'" value="C" >'."	".$row['Option_3'].'</input></br>
							D. <input type="radio" id="D" onClick="Mark_choice_current_question()" name="'.$row['id'].'" value="D" >'."	".$row['Option_4'].'</input></br>
